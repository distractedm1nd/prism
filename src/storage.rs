...
pub trait Database: Send + Sync {
    ...
    fn get_epoch_operation(&self) -> PrismResult<u64>;
    fn set_epoch(&self, epoch: &u64) -> PrismResult<()>;
    fn reset_epoch_operation_counter(&self) -> PrismResult<()>;
    fn update_hashchain(
        &self,
    ...
    fn increment_epoch_operation(&self) -> PrismResult<u64>;
    fn add_merkle_proof(
        &self,
        epoch: &u64,
        epoch_operation: &u64,
...
impl Database for RedisConnection {
    ...
    fn get_hashchain(&self, key: &str) -> PrismResult<Vec<ChainEntry>> {
        ...
        let value: String = con.get(format!("main:{}", key)).map_err(|_| {
            PrismError::Database(DatabaseError::NotFoundError(format!("key: {}", key)))
        })?;
    ...
    fn get_epoch_operation(&self) -> PrismResult<u64> {
        ...
        con.get("app_state:epoch_operation").map_err(|_| {
            PrismError::Database(DatabaseError::NotFoundError("epoch operation".to_string()))
        })
    }
    ...
    fn reset_epoch_operation_counter(&self) -> PrismResult<()> {
        ...
        con.set::<&str, &u64, ()>("app_state:epoch_operation", &0)
            .map_err(|_| {
                PrismError::Database(DatabaseError::WriteError("epoch_operation->0".to_string()))
            })
    }
    ...
    fn increment_epoch_operation(&self) -> PrismResult<u64> {
        ...
        con.incr::<&'static str, u64, u64>("app_state:epoch_operation", 1)
            .map_err(|_| {
                PrismError::Database(DatabaseError::WriteError("incremented epoch".to_string()))
            })
    }
